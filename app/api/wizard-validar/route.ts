...
export async function POST(req: NextRequest) {
  ...
  const systemPrompt = `Sos un coach de planificación basado en la Serie de Objetivos de L. Ronald Hubbard. Tu función es evaluar si el contenido de cada paso del wizard cumple con los principios de la Serie. Respondé ÚNICAMENTE con JSON válido, sin markdown ni texto adicional. Sé constructivo — el objetivo es ayudar a mejorar, no bloquear.`

  let userPrompt = ''

  if (paso === 1) {
    userPrompt = `Evaluá esta situación según la Serie de Objetivos de LRH.
La Serie dice: "Un programa debe manejar situaciones verdaderas — las situaciones que reducen la producción y la prosperidad. Hacer cualquier otra cosa es omitir pasos en la secuencia."
...
NO generes observaciones si la situación es específica y concreta.`
    ...
    userPrompt = `Evaluá este propósito según la Serie de Objetivos de LRH.
La Serie dice: "Los propósitos tienen que ejecutarse. Son algo que HACER. Detrás de cada paro hay un propósito fallido."
...
Si está bien: observaciones null. Si se puede mejorar: texto corto de 1-2 oraciones.`
    ...
    userPrompt = `Evaluá el nombre y objetivo mayor de este programa según la Serie de Objetivos de LRH.
La Serie dice: "El Objetivo Mayor es la aspiración general y amplia que posiblemente abarca un período de tiempo largo y aproximado."
