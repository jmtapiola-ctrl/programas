  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-4">
        ...
        {/* Error banner */}
        {error && (
          <div className="flex items-center gap-3 rounded-lg border border-red-800 bg-red-950/50 px-4 py-3 text-[17px] text-red-300">
            <span className="flex-1">{error}</span>
            {pendingMessage && (
              <button
                onClick={onRetry}
                className="flex-shrink-0 rounded-md bg-red-800 px-3 py-1 text-[15px] font-medium text-red-100 hover:bg-red-700 transition-colors"
              >
                Reintentar
              </button>
            )}
...
function Burbuja({
  ...
  return (
    <div className={`flex ${isModel ? 'justify-start' : 'justify-end'}`}>
      <div
        className={`max-w-[85%] rounded-xl px-4 py-3 text-[17px] leading-relaxed ${
          isModel
      ...
      >
        {contenido ? (
          <ReactMarkdown
            components={{
              ...
              h1: ({ children }) => <h1 className="text-[26px] font-bold text-foreground mt-4 mb-2 first:mt-0">{children}</h1>,
              h2: ({ children }) => <h2 className="text-[21px] font-semibold text-foreground mt-3 mb-2 first:mt-0">{children}</h2>,
              h3: ({ children }) => <h3 className="text-[18px] font-semibold text-foreground/95 mt-2 mb-1.5 first:mt-0">{children}</h3>,
              h4: ({ children }) => <h4 className="text-[16px] font-semibold uppercase tracking-wide text-muted-foreground mt-2 mb-1 first:mt-0">{children}</h4>,
              blockquote: ({ children }) => (
              ...
              code: ({ children, ...props }) => {
                ...
                if (inline) {
                  return (
                    <code className="rounded bg-foreground/10 px-1 py-0.5 text-[16px] font-mono">{children}</code>
                  )
                }
                return <code className="font-mono text-[16px]">{children}</code>
              },
              pre: ({ children }) => (
                <pre className="my-2 overflow-x-auto rounded-md bg-foreground/10 p-3 text-[16px] leading-relaxed">{children}</pre>
              ),
              a: ({ children, href }) => (
        ...
        {streaming && persisting && (
          <p className="mt-2 text-[14px] text-muted-foreground/70 italic flex items-center gap-2">
